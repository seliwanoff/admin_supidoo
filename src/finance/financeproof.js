    ...
    if (state.customtype === "Working Capital") {
      ...
      await axios
        ...
        .then((res) => {
          ...
          const invoiceDocument = res.data.data.invoiceDocument;
          const extractedData = [agreement, invoiceDocument];
          const filteredDocumentsInvoice = Object.values(extractedData).filter(
            (doc) => !doc?.hasOwnProperty("status")
          );
          //  console.log(filteredDocumentsInvoice);
          setPendingDocuments(filteredDocumentsInvoice);
          setisLoading(false);
        })
        .catch((e) => {
          console.log(e);
      ...
      if (state.customtype === "Working Capital") {
        ...
        await axios
          ...
          .then((res) => {
            ...
            const invoiceDocument = res.data.data.invoiceDocument;
            const extractedData = [agreement, invoiceDocument];
            const filteredDocumentsInvoice = Object.values(
              extractedData
            ).filter((doc) => !doc?.hasOwnProperty("status"));
            //  console.log(filteredDocumentsInvoice);
            setPendingDocuments(filteredDocumentsInvoice);
    ...
    if (state?.seg === "single") {
      ...
      if (state?.customtype === "Credit Line") {
        ...
        const extractedData = [
          ...
          { type: "invoiceDocument", data: invoiceDocument },
        ];
        // console.log(extractedData);
        extractedData.forEach((item, index) => {
          if (item.data && item.data.status) {
            extractedData.splice(index, 1); // Remove the item if it has a status property
          }
        });

        //console.log(extractedData);
        for (const key in extractedData) {
          if (!extractedData[key].hasOwnProperty("status")) {
            console.log(key);
            return extractedData[key].type;
          }
        }
    ...
    <div className="">
      ...
      <div className="main_bd_lg">
        ...
        <div className="mg-lgbg">
          {isLoading === false && pendingDocuments.length > 0 ? (
            <embed
              src={`${
                state?.seg === "single"
                  ? state?.image
